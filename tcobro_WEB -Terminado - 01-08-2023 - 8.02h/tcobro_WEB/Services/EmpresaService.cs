    ...
    public class EmpresaService : BaseService, IEmpresaService
    {
        ...
        public Task<T> Actualizar<T>(EmpresaUpdateDTO dto, string token)
        {
            return SendAsync<T>(new APIRequest()
            {
                ...
                Url = _empresaUrl + "/api/v1/Empresa/" + dto.Id, //Se conecta con la API
        ...
        public Task<T> Crear<T>(EmpresaCreateDTO dto, string token)
        {
            return SendAsync<T>(new APIRequest()
            {
                ...
                Datos = dto, 
                Url = _empresaUrl + "/api/v1/Empresa",
                Token = token
        ...
        public Task<T> Obtener<T>(int id, string token)
        {
            return SendAsync<T>(new APIRequest()
            {
                ...
                Url = _empresaUrl + "/api/v1/Empresa/" + id,
                Token = token
            });
        }
        ...
        public Task<T> ObtenerTodosPaginado<T>(string token, int numeroDePagina = 1, int registrosPorPagina = 4)
        {
            return SendAsync<T>(new APIRequest()
            {
                APITipo = DefinicionesEstaticas.APITipo.GET,
                Url = _empresaUrl + "/api/v1/Empresa/EmpresasPaginado",
                Token = token,
                Parametros = new Parametros() { NumeroDePagina = numeroDePagina , RegistrosPorPagina = registrosPorPagina }
            });
        }
