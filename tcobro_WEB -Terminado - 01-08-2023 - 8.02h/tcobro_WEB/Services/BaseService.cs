    ...
    public class BaseService : IBaseService
    {
        ...
        public async Task<T> SendAsync<T>(APIRequest apiRequest)
        {
            try
            {
                ...
                //Comprueba si tenemos llenas los datos de las propiedades de apiRequest
                if (apiRequest != null)
                {
                    message.RequestUri = new Uri(apiRequest.Url); //Obtiene la URL por la cual nos queremos conectar
                }
                else //De lo contrario, se construye la URL
                {
                    var builder = new UriBuilder(apiRequest.Url);
                    var query = HttpUtility.ParseQueryString(builder.Query);

                    query["NumeroDePagina"] = apiRequest.Parametros.NumeroDePagina.ToString();
                    query["RegistrosPorPagina"] = apiRequest.Parametros.RegistrosPorPagina.ToString();

                    builder.Query = query.ToString();

                    //Variable de tipo string
                    string url = builder.ToString();// Forma ->     api/Empresa/EmpresaPaginado/NumeroDePagina=1&RegistrosPorPagina=4
                ...
                try
                {
                    ...
                    if(response != null &&                       
                      (apiResponse.StatusCode == HttpStatusCode.BadRequest || apiResponse.StatusCode == HttpStatusCode.NotFound))
                    {
