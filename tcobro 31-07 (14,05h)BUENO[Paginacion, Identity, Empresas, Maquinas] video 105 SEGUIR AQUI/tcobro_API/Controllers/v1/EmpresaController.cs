...
namespace tcobro_API.Controllers.v1
{
    ...
    public class EmpresaController : ControllerBase
    {
        ...
        [Authorize(Roles = "admin", AuthenticationSchemes = "Bearer")]//Usuario autorizado para acceder al end-point
        ...
        [HttpGet("EmpresasPaginado")]//Agrega a la ruta /EmpresasPaginado
        [ResponseCache(CacheProfileName = "Default30")]//Cada 30 segundos no consulta la informacion a la BBDD, envia la info que tiene en memoria de cache,configurado en program.cs
        [ProducesResponseType(StatusCodes.Status200OK)] //Documenta el codigo de estado
        public ActionResult<APIResponse> GetEmpresasPaginado([FromQuery] Parametros parametros)
        {
            try
            {
                var empresaList = _empresaRepositorio.ObtenerTodosPaginado(parametros);//Obtiene la lista de empresas
                ...
                {
                    _response.PaginasTotales = 1;
                }
                else
                {
                    _response.PaginasTotales = empresaList.MetaData.PaginasTotales;
                }
                return Ok(_response);
        ...
        //Crear una empresa
        [HttpPost]
        [Authorize(Roles = "admin", AuthenticationSchemes = "Bearer")]//Unicamente siendo admin se puede acceder al end-point
